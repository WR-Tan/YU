...
#import "PAFFRefreshStateHeader.h"

@interface PAFFRefreshStateHeader () {
  /** 显示上一次刷新时间的label */
...
@implementation PAFFRefreshStateHeader
#pragma mark - 懒加载
...
- (void)setLastUpdatedTimeKey:(NSString *)lastUpdatedTimeKey {
  ...
  NSDate *lastUpdatedTime =
      [[NSUserDefaults standardUserDefaults] objectForKey:lastUpdatedTimeKey];
  ...
  if (lastUpdatedTime) {
    ...
    NSDateComponents *cmp1 =
        [calendar components:unitFlags fromDate:lastUpdatedTime];
...
- (void)prepare {
  ...
  // 初始化文字
  [self setTitle:PAFFRefreshHeaderIdleText forState:PAFFRefreshStateIdle];
  [self setTitle:PAFFRefreshHeaderPullingText forState:PAFFRefreshStatePulling];
  [self setTitle:PAFFRefreshHeaderRefreshingText
