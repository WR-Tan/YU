...
#import "PAFFRefreshAutoFooter.h"

@interface PAFFRefreshAutoFooter ()
@end

@implementation PAFFRefreshAutoFooter
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change {
  ...
  if (self.state != PAFFRefreshStateIdle || !self.automaticallyRefresh ||
      self.mj_y == 0)
    return;
    
  if (_scrollView.mj_insetT + _scrollView.mj_contentH >
      _scrollView.mj_h) { // 内容超过一个屏幕
    // 这里的_scrollView.mj_contentH替换掉self.mj_y更为合理
    if (_scrollView.mj_offsetY >=
        _scrollView.mj_contentH - _scrollView.mj_h +
            self.mj_h * self.appearencePercentTriggerAutoRefresh +
...
- (void)scrollViewPanStateDidChange:(NSDictionary *)change {
  ...
  if (_scrollView.panGestureRecognizer.state ==
      UIGestureRecognizerStateEnded) { // 手松开
    if (_scrollView.mj_insetT + _scrollView.mj_contentH <=
        _scrollView.mj_h) { // 不够一个屏幕
