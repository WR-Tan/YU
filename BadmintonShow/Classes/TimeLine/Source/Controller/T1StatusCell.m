...
- (void)setWithMedias:(NSArray *)medias {
    for (int i = 0; i < 4; i++) {
        ...
        if (i >= medias.count) {
            ...
            view.hidden = NO;
            BSTLMedia *media = medias[i];
            [view setImage:[BSTLHelper imageNamed:@"bs_def_userAvatar"]];
//            [view setImageWithURL:media.url  
//                      placeholder:nil
...
//    return self;
//}
//- (void)setWithLayout:(BSTLLayout *)layout {
...
- (instancetype)init {
    ...
    [_replyButton setImage:[BSTLHelper imageNamed:@"icn_tweet_action_inline_reply_off"] forState:UIControlStateNormal];
    ...
    _retweetImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_retweet_off"];
    ...
    _favoriteImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_favorite_off"];
    ...
    [_followButton setImage:[BSTLHelper imageNamed:@"icn_tweet_action_inline_follow_off_ipad_experiment"] forState:UIControlStateNormal];
...
- (void)setWithLayout:(BSTLLayout *)layout {
    BSTLModel *tweet = layout.displayedTweet;
//    if (tweet.retweeted) {
//        _retweetImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_retweet_on_white"];
//    } else {
        _retweetImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_retweet_off"];
//    }
    
    if (tweet.favorited) {
        _favoriteImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_favorite_on_white"];
    } else {
        _favoriteImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_favorite_off"];
    }
...
//    if (tweet.user.following) {
        _followButton.hidden = YES;  // don't show _followButton here
//    } else {
...
- (void)updateRetweetWithAnimation {
    BSTLLayout *layout = self.cell.layout;
//    BSTLModel *tweet = layout.displayedTweet;
//    if (tweet.retweeted) {
//        _retweetImageView.image = [BSTLHelper imageNamed:@"icn_tweet_action_inline_retweet_on_white"];
//    } else {
...
- (void)updateFavouriteWithAnimation {
    ...
    BSTLModel *tweet = layout.displayedTweet;
    if (tweet.favorited) {
        UIImage *img = [BSTLHelper imageNamed:@"fav02c-sheet"];
...
//    BSTLModel * tweet = layout.displayedTweet;
//    if (tweet.user.following) {
//        [_followButton setImage:[BSTLHelper imageNamed:@"icn_tweet_action_inline_follow_on_ipad_experiment"] forState:UIControlStateNormal];
//    } else {
        [_followButton setImage:[BSTLHelper imageNamed:@"icn_tweet_action_inline_follow_off_ipad_experiment"] forState:UIControlStateNormal];
//    }
}
...
//            self.backgroundColor = [UIColor clearColor];
        ...
        if (state == YYGestureRecognizerStateEnded) {
            self.backgroundColor = [UIColor whiteColor]; //by lzh
            UITouch *t = touches.anyObject;
            CGPoint p = [t locationInView:self];
            if (CGRectContainsPoint(self.bounds, p)) {
...
//    }
    ...
    [_avatarView.layer setImageWithURL:tweet.user.avatarUrl options:YYWebImageOptionSetImageWithFadeAnimation];
...
//            _socialIconView.hidden = NO;
//            _socialIconView.image = [BSTLHelper imageNamed:@"icn_social_proof_conversation_default"];
//        } else if (layout.tweet.inReplyToScreenName) {
//            _socialIconView.hidden = NO;
//            _socialIconView.image = [BSTLHelper imageNamed:@"icn_activity_rt_tweet"];
//        } else {
...
//    }
    ...
    self.conversationTopJoin.hidden =  YES; //!layout.showConversationTopJoin;
    self.conversationBottomJoin.hidden =  YES; //!layout.showConversationBottomJoin;
...
- (void)setLayout:(BSTLLayout *)layout {
    _layout = layout;
