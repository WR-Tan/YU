...
@implementation BSStatusTitleView
- (instancetype)initWithFrame:(CGRect)frame {
    if (frame.size.width == 0 && frame.size.height == 0) {
        frame.size.width = kScreenWidth;
        frame.size.height = kBSCellTitleHeight;
    }
    self = [super initWithFrame:frame];
    _titleLabel = [YYLabel new];
    _titleLabel.size = CGSizeMake(kScreenWidth - 100, self.height);
    _titleLabel.left = kBSCellPadding;
    ...
    line.backgroundColor = kBSCellLineColor.CGColor;
...
@implementation BSStatusProfileView {
    BOOL _trackingTouch;
}
- (instancetype)initWithFrame:(CGRect)frame {
    if (frame.size.width == 0 && frame.size.height == 0) {
        frame.size.width = kScreenWidth;
        frame.size.height = kBSCellProfileHeight;
    }
    self = [super initWithFrame:frame];
    self.exclusiveTouch = YES;
    @weakify(self);
    ...
    _avatarView.origin = CGPointMake(kBSCellPadding, kBSCellPadding + 3);
    ...
    _nameLabel.size = CGSizeMake(kBSCellNameWidth, 24);
    _nameLabel.left = _avatarView.right + kBSCellNamePaddingLeft;
...
- (void)setVerifyType:(BSUserVerifyType)verifyType {
    _verifyType = verifyType;
    switch (verifyType) {
        case BSUserVerifyTypeStandard: {
            _avatarBadgeView.hidden = NO;
            _avatarBadgeView.image = [BSStatusHelper imageNamed:@"avatar_vip"];
        } break;
        case BSUserVerifyTypeClub: {
            _avatarBadgeView.hidden = NO;
            _avatarBadgeView.image = [BSStatusHelper imageNamed:@"avatar_grassroot"];
        } break;
        default: {
...
@implementation BSStatusCardView {
    BOOL _isRetweet;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    self.backgroundColor = kBSCellInnerViewColor;
    self.layer.borderWidth = CGFloatFromPixel(1);
    self.layer.borderColor = [UIColor colorWithWhite:0.000 alpha:0.070].CGColor;
    return self;
}

- (void)setWithLayout:(BSStatusLayout *)layout isRetweet:(BOOL)isRetweet {
//    BSPageInfo *pageInfo = isRetweet ? layout.status.retweetedStatus.pageInfo : layout.status.pageInfo;
//    if (!pageInfo) return;
...
//        case BSStatusCardTypeNone: {
//            
//        } break;
//        case BSStatusCardTypeNormal: {
//            self.width = kBSCellContentWidth;
...
//            [_button setImage:[BSStatusHelper imageNamed:@"multimedia_videocard_play"] forState:UIControlStateNormal];
//            
//        } break;
//        default: {
//            
...
//    }
//    
//    self.backgroundColor = isRetweet ? [UIColor whiteColor] : kBSCellInnerViewColor;
}

- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    self.backgroundColor = kBSCellInnerViewHighlightColor;
}

- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    self.backgroundColor = _isRetweet ? [UIColor whiteColor] : kBSCellInnerViewColor;
...
@implementation BSStatusToolbarView
- (instancetype)initWithFrame:(CGRect)frame {
    if (frame.size.width == 0 && frame.size.height == 0) {
        frame.size.width = kScreenWidth;
        frame.size.height = kBSCellToolbarHeight;
    }
    self = [super initWithFrame:frame];
    self.exclusiveTouch = YES;
...
//    _repostButton.hidden = YES;
    ...
    _commentImageView = [[UIImageView alloc] initWithImage:[BSStatusHelper imageNamed:@"timeline_icon_comment"]];
    _commentImageView.centerY = self.height / 2;
    [_commentButton addSubview:_commentImageView];
    _likeImageView = [[UIImageView alloc] initWithImage:[BSStatusHelper imageNamed:@"timeline_icon_unlike"]];
    ...
    _topLine.backgroundColor = kBSCellLineColor.CGColor;
    ...
    [_repostButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        BSStatusCell *cell = weak_self.cell;
...
- (void)setWithLayout:(BSStatusLayout *)layout {
    ...
    _likeImageView.image = layout.status.favorited ? [self likeImage] : [self unlikeImage];
}
...
- (UIImage *)likeImage {
    ...
    dispatch_once(&onceToken, ^{
        img = [BSStatusHelper imageNamed:@"timeline_icon_like"];
    });
    return img;
}
...
- (UIImage *)unlikeImage {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        img = [BSStatusHelper imageNamed:@"timeline_icon_unlike"];
    });
    return img;
}
...
- (void)setLiked:(BOOL)liked withAnimation:(BOOL)animation {
    BSStatusLayout *layout = _cell.statusView.layout;
    if (layout.status.favorited == liked) return;
    
    UIImage *image = liked ? [self likeImage] : [self unlikeImage];
    int newCount = layout.status.favoriteCount;
    newCount = liked ? newCount + 1 : newCount - 1;
    if (newCount < 0) newCount = 0;
    if (liked && newCount < 1) newCount = 1;
    NSString *newCountDesc = newCount > 0 ? [BSStatusHelper shortedNumberDesc:newCount] : @"赞";
    ...
    likeText.font = font;
    likeText.color = liked ? kBSCellToolbarTitleHighlightColor : kBSCellToolbarTitleColor;
    YYTextLayout *textLayout = [YYTextLayout layoutWithContainer:container text:likeText];
    
    layout.status.favorited = liked;
    layout.status.favorited = newCount;
...
@implementation BSStatusTagView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    @weakify(self);
    _button = [UIButton buttonWithType:UIButtonTypeCustom];
    [_button setBackgroundImage:[UIImage imageWithColor:kBSCellBackgroundColor] forState:UIControlStateNormal];
    ...
    _imageView = [UIImageView new];
    _imageView.size = CGSizeMake(kBSCellTagPlaceHeight, kBSCellTagPlaceHeight);
    _imageView.image = [BSStatusHelper imageNamed:@"timeline_icon_locate"];
    _imageView.hidden = YES;
    [self addSubview:_imageView];
    
    _label.height = kBSCellTagPlaceHeight;
    _button.height = kBSCellTagPlaceHeight;
    self.height = kBSCellTagPlaceHeight;
    return self;
}

- (void)setWithLayout:(BSStatusLayout *)layout {
    if (layout.tagType == BSStatusTagTypePlace) {
        _label.height = kBSCellTagPlaceHeight;
        _imageView.hidden = NO;
        _button.hidden = NO;
    ...
    } else if (layout.tagType == BSStatusTagTypeNormal) {
        _imageView.hidden = YES;
        _button.hidden = YES;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _titleView = [BSStatusTitleView new];
    _titleView.hidden = YES;
    [_contentView addSubview:_titleView];
    
    _profileView = [BSStatusProfileView new];
    ...
    [_menuButton setImage:[BSStatusHelper imageNamed:@"timeline_icon_more"] forState:UIControlStateNormal];
    [_menuButton setImage:[BSStatusHelper imageNamed:@"timeline_icon_more_highlighted"] forState:UIControlStateHighlighted];
    ...
    _retweetBackgroundView.backgroundColor = kBSCellInnerViewColor;
    _retweetBackgroundView.width = kScreenWidth;
    [_contentView addSubview:_retweetBackgroundView];
    
    _textLabel = [YYLabel new];
    _textLabel.left = kBSCellPadding;
    _textLabel.width = kBSCellContentWidth;
    ...
    _retweetTextLabel.left = kBSCellPadding;
    _retweetTextLabel.width = kBSCellContentWidth;
    ...
    for (int i = 0; i < 9; i++) {
        ...
        imageView.backgroundColor = kBSCellHighlightColor;
    ...
    _cardView = [BSStatusCardView new];
    _cardView.hidden = YES;
    [_contentView addSubview:_cardView];
    
    _tagView = [BSStatusTagView new];
    _tagView.left = kBSCellPadding;
    _tagView.hidden = YES;
    [_contentView addSubview:_tagView];
    
    _toolbarView = [BSStatusToolbarView new];
...
- (void)setLayout:(BSStatusLayout *)layout {
    ...
    /// 圆角头像
    [_profileView.avatarView setImageWithURL:layout.status.user.avatarUrl //profileImageURL
                                 placeholder:nil
                                     options:kNilOptions
                                     manager:[BSStatusHelper avatarImageManager] //< 圆角头像manager，内置圆角处理
                                    progress:nil
                                   transform:nil
                                  completion:nil];
...
//    NSURL *picBg = [BSStatusHelper defaultURLForImageURL:layout.status.picBg];
...
//    }];
    ...
    if (layout.tagHeight > 0) {
        ...
        _tagView.centerY = _contentView.height - kBSCellToolbarHeight - layout.tagHeight / 2;
    } else {
        _tagView.hidden = YES;
    }
...
//    NSArray *pics = isRetweet ? _layout.status.retweetedStatus.pics : _layout.status.pics;
    NSArray *pics = _layout.status.medias;
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (i >= picsCount) {
            ...
            switch (picsCount) {
                case 1: {
                    origin.x = kBSCellPadding;
                    origin.y = imageTop;
                } break;
                case 4: {
                    origin.x = kBSCellPadding + (i % 2) * (picSize.width + kBSCellPaddingPic);
                    origin.y = imageTop + (int)(i / 2) * (picSize.height + kBSCellPaddingPic);
                } break;
                default: {
                    origin.x = kBSCellPadding + (i % 3) * (picSize.width + kBSCellPaddingPic);
                    origin.y = imageTop + (int)(i / 3) * (picSize.height + kBSCellPaddingPic);
                } break;
            }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (!_retweetBackgroundView.hidden && insideRetweet) {
        [(_retweetBackgroundView) performSelector:@selector(setBackgroundColor:) withObject:kBSCellHighlightColor afterDelay:0.15];
        _touchRetweetView = YES;
    } else {
        [(_contentView) performSelector:@selector(setBackgroundColor:) withObject:kBSCellHighlightColor afterDelay:0.15];
...
- (void)touchesRestoreBackgroundColor {
    [NSObject cancelPreviousPerformRequestsWithTarget:_retweetBackgroundView selector:@selector(setBackgroundColor:) object:kBSCellHighlightColor];
    [NSObject cancelPreviousPerformRequestsWithTarget:_contentView selector:@selector(setBackgroundColor:) object:kBSCellHighlightColor];
...
@implementation BSStatusCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    _statusView = [BSStatusView new];
