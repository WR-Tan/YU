...
#import "YYKit.h"
#import "BSTLModel.h"
#import "BSStatusHelper.h"

// 宽高
#define kBSCellTopMargin 8      // cell 顶部灰色留白
#define kBSCellTitleHeight 36   // cell 标题高度 (例如"仅自己可见")
#define kBSCellPadding 12       // cell 内边距
#define kBSCellPaddingText 10   // cell 文本与其他元素间留白
#define kBSCellPaddingPic 4     // cell 多张图片中间留白
#define kBSCellProfileHeight 56 // cell 名片高度
#define kBSCellCardHeight 70    // cell card 视图高度
#define kBSCellNamePaddingLeft 14 // cell 名字和 avatar 之间留白
#define kBSCellContentWidth (kScreenWidth - 2 * kBSCellPadding) // cell 内容宽度
#define kBSCellNameWidth (kScreenWidth - 110) // cell 名字最宽限制

#define kBSCellTagPadding 8         // tag 上下留白
#define kBSCellTagNormalHeight 16   // 一般 tag 高度
#define kBSCellTagPlaceHeight 24    // 地理位置 tag 高度

#define kBSCellToolbarHeight 35     // cell 下方工具栏高度
#define kBSCellToolbarBottomMargin 2 // cell 下方灰色留白
...
#define kBSCellToolbarFontSize 14 // 工具栏字体大小

// 颜色
#define kBSCellNameNormalColor UIColorHex(333333) // 名字颜色
#define kBSCellNameOrangeColor UIColorHex(f26220) // 橙名颜色 (VIP)
#define kBSCellTimeNormalColor UIColorHex(828282) // 时间颜色
#define kBSCellTimeOrangeColor UIColorHex(f28824) // 橙色时间 (最新刷出)

#define kBSCellTextNormalColor UIColorHex(333333) // 一般文本色
#define kBSCellTextSubTitleColor UIColorHex(5d5d5d) // 次要文本色
#define kBSCellTextHighlightColor UIColorHex(527ead) // Link 文本色
#define kBSCellTextHighlightBackgroundColor UIColorHex(bfdffe) // Link 点击背景色
#define kBSCellToolbarTitleColor UIColorHex(929292) // 工具栏文本色
#define kBSCellToolbarTitleHighlightColor UIColorHex(df422d) // 工具栏文本高亮色

#define kBSCellBackgroundColor UIColorHex(f2f2f2)    // Cell背景灰色
#define kBSCellHighlightColor UIColorHex(f0f0f0)     // Cell高亮时灰色
#define kBSCellInnerViewColor UIColorHex(f7f7f7)   // Cell内部卡片灰色
#define kBSCellInnerViewHighlightColor  UIColorHex(f0f0f0) // Cell内部卡片高亮时灰色
#define kBSCellLineColor [UIColor colorWithWhite:0.000 alpha:0.09] //线条颜色

#define kBSLinkHrefName @"href" //NSString
#define kBSLinkURLName @"url" //BSURL
#define kBSLinkTagName @"tag" //BSTag
#define kBSLinkTopicName @"topic" //BSTopic
#define kBSLinkAtName @"at" //NSString

/// 风格
typedef NS_ENUM(NSUInteger, BSLayoutStyle) {
    BSLayoutStyleTimeline = 0, ///< 时间线 (目前只支持这一种)
    BSLayoutStyleDetail,       ///< 详情页
};

/// 卡片类型 (这里随便写的，只适配了微博中常见的类型)
typedef NS_ENUM(NSUInteger, BSStatusCardType) {
    BSStatusCardTypeNone = 0, ///< 没卡片
    BSStatusCardTypeNormal,   ///< 一般卡片布局
    BSStatusCardTypeVideo,    ///< 视频
};

/// 最下方Tag类型，也是随便写的，微博可能有更多类型同时存在等情况
typedef NS_ENUM(NSUInteger, BSStatusTagType) {
    BSStatusTagTypeNone = 0, ///< 没Tag
    BSStatusTagTypeNormal,   ///< 文本
    BSStatusTagTypePlace,    ///< 地点
};
...
/**
 ...
 布局排版应该在后台线程完成。
 */
@interface BSStatusLayout : NSObject
- (instancetype)initWithStatus:(BSTLModel *)status style:(BSLayoutStyle)style;
- (void)layout; ///< 计算布局
- (void)updateDate; ///< 更新时间字符串

// 以下是数据
@property (nonatomic, strong) BSTLModel *status;
@property (nonatomic, assign) BSLayoutStyle style;
...
@property (nonatomic, assign) BSStatusCardType retweetCardType;
...
@property (nonatomic, assign) BSStatusCardType cardType;
...
@property (nonatomic, assign) BSStatusTagType tagType;
 ...
 */
@interface BSTextLinePositionModifier : NSObject <YYTextLinePositionModifier>
