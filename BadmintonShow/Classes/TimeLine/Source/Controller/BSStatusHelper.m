...
+ (NSArray *)emoticonGroups {
    ...
    dispatch_once(&onceToken, ^{
        ...
        groups = (NSMutableArray *)[NSArray modelArrayWithClass:[BSEmoticonGroup class] json:packages];
        
        NSMutableDictionary *groupDic = [NSMutableDictionary new];
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            BSEmoticonGroup *group = groups[i];
        ...
        for (NSString *path in additionals) {
            BSEmoticonGroup *group = groupDic[path];
            if (!group) continue;
            ...
            BSEmoticonGroup *addGroup = [BSEmoticonGroup modelWithJSON:infoJSON];
            if (addGroup.emoticons.count) {
                for (BSEmoticon *emoticon in addGroup.emoticons) {
