...
@implementation BSTLLayout


- (void)setTweet:(BSTLModel *)tweet {
    if (_tweet != tweet) {
        _tweet = tweet;
        [self layout];
    }
...
//    }
    ...
    BSTLModel *tweet = self.displayedTweet;
    
    UIFont *nameSubFont = [UIFont systemFontOfSize:kT1UserNameSubFontSize];
    NSMutableAttributedString *dateText = [[NSMutableAttributedString alloc] initWithString:[BSTLHelper stringWithTimelineDate:tweet.createdAt]];
    
//    if (tweet.card) {
//        UIImage *iconImage = [BSTLHelper imageNamed:@"ic_tweet_attr_summary_default"];
...
//    }
    ...
    NSMutableAttributedString *nameText = [[NSMutableAttributedString alloc] initWithString:(tweet.user.userName ? tweet.user.userName : @"")];
    nameText.font = nameFont;
    nameText.color = kT1UserNameColor;
    if (tweet.user.nickName) {
        NSMutableAttributedString *screenNameText = [[NSMutableAttributedString alloc] initWithString:tweet.user.nickName];
...
//        NSMutableSet *imageIDs = [NSMutableSet new];
        ...
        for (BSTLMedia *media in tweet.medias) {
...
//        BSTLModelquote = _tweet.quotedStatus;
...
- (NSAttributedString *)textForTweet:(BSTLModel *)tweet{
...
- (BSTLModel *)displayedTweet {
    return _tweet /* .retweetedStatus ? _tweet.retweetedStatus : _tweet */;
}

- (YYTextLayout *)retweetCountTextLayoutForTweet:(BSTLModel *)tweet {
    //  not retweet in this model
//    if (tweet.retweetCount > 0) {
//        NSMutableAttributedString *retweet = [[NSMutableAttributedString alloc] initWithString:[BSTLHelper shortedNumberDesc:tweet.retweetCount]];
...
- (YYTextLayout *)favoriteCountTextLayoutForTweet:(BSTLModel *)tweet {
    if (tweet.favoriteCount > 0) {
        NSMutableAttributedString *favourite = [[NSMutableAttributedString alloc] initWithString:[BSTLHelper shortedNumberDesc:tweet.favoriteCount]];
