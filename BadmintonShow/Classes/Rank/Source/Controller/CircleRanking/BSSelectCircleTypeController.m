...
//#warning 暂时只做公开的吧，私密的下个版本做。    
   ...
   static NSString *openDesc = @"        每个人都可以加入到你创建的公开圈子，前提是他/她的分类圈子(比如：公司)还没有加入其他公司的圈子。如果要加入另一个，必须退出之前对应分类圈子";
    ...
    cell.nameLabel.text = indexPath.row ? openName : privateName;
    cell.descLabel.text = indexPath.row ? openDesc : privateDesc;
    cell.markLable.text = indexPath.row ? openMark : privateMark;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BSCircleOpenType type = indexPath.row ? BSCircleOpenTypeOpen : BSCircleOpenTypePrivate;
